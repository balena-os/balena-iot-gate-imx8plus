...
IOT_GATE_IMX8PLUS_UUU = 'Connect the debug UART and PROG ports to your PC. Use "sudo <a href="https://github.com/nxp-imx/mfgtools">uuu</a> imx-boot" to load imx-boot_4g on your device and then interrupt u-boot in cmdline. You can skip using uuu if your device already runs an u-boot that has console access and the ums command enabled.'
...
IOT_GATE_IMX8PLUS_FLASH_IMAGE = 'Mount the device\'s eMMC from u-boot using \'ums 0 mmc 2\' and write the downloaded balenaOS image to it. Both the bootloader and the OS image should be written before any reboot.'
